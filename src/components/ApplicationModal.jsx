...
function ApplicationModal({ show, onClose, onSubmit, editingApp, statuses, userId }) {
  ...
  // Load user-specific resumes and cover letters
  useEffect(() => {
    getCurrentUser()
      .then(u => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-[680px] mx-4">
        ...
        <div className="px-7 py-5">
          ...
          {activeTab === 'basic' && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="dateApplied" value={formData.dateApplied} onChange={handleChange} type="date"
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>
              <div className="col-span-2">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Job Posting URL</label>
                <input name="jobUrl" value={formData.jobUrl} onChange={handleChange}
                  placeholder="e.g. https://shopify.com/careers/..."
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>
              <div className="col-span-2">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Notes</label>
          ...
          {activeTab === 'docs' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Resume Used</label>
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Cover Letter Used</label>
              ...
              {(formData.resumeUsed || formData.coverLetterUsed) && (
                <div className="col-span-2 bg-blue-50 rounded-xl p-4">
                  <p className="text-xs font-bold text-blue-600 mb-2">📎 Documents attached</p>
          ...
          {activeTab === 'schedule' && (
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Application Deadline</label>
                ...
                {formData.deadline && (
                  <p className="text-xs mt-1 font-semibold">
                    {(() => {
                      ...
                      if (days <= 3)  return <span className="text-orange-500">⚠️ {days} day{days>1?'s':''} left — apply soon!</span>
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Interview Date</label>
                <input name="interviewDate" value={formData.interviewDate} onChange={handleChange} type="date"
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Interview Time</label>
                <input name="interviewTime" value={formData.interviewTime} onChange={handleChange} type="time"
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>

              {formData.interviewDate && (
                <div className="col-span-2 bg-orange-50 border border-orange-200 rounded-xl p-4">
                  <p className="text-xs font-bold text-orange-600 mb-1">🎤 Interview Countdown</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <div className="bg-blue-50 rounded-xl p-4 mb-2">
                  <p className="text-xs font-bold text-blue-600 mb-1">👤 Why track recruiter contacts?</p>
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Recruiter Name</label>
                <input name="recruiterName" value={formData.recruiterName} onChange={handleChange}
                  placeholder="e.g. Sarah Johnson"
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Recruiter Email</label>
                <input name="recruiterEmail" value={formData.recruiterEmail} onChange={handleChange}
                  placeholder="e.g. sarah@shopify.com" type="email"
                  className="mt-1 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-400 transition-colors"/>
              </div>
              <div className="col-span-2">
              ...
              {(formData.recruiterName || formData.recruiterEmail) && (
                <div className="col-span-2 bg-gray-50 rounded-xl p-4">
                  <p className="text-xs font-bold text-gray-500 mb-2">Contact Summary</p>
                  {formData.recruiterName  && <p className="text-sm text-gray-700">👤 {formData.recruiterName}</p>}
                  {formData.recruiterEmail && <p className="text-sm text-gray-700">📧 {formData.recruiterEmail}</p>}
