...
function Dashboard({ statuses, thresholds }) {
  ...
  const colorMap = {
    'Applied':           { color: 'text-blue-500',   bg: 'bg-white', border: 'border-blue-200',   dot: 'bg-blue-500'   },
    'Under Review':      { color: 'text-sky-600',    bg: 'bg-white', border: 'border-sky-200',    dot: 'bg-sky-600'    },
    'Phone Screen':      { color: 'text-purple-600', bg: 'bg-white', border: 'border-purple-200', dot: 'bg-purple-600' },
    'Technical Test':    { color: 'text-amber-600',  bg: 'bg-white', border: 'border-amber-200',  dot: 'bg-amber-600'  },
    'Interview':         { color: 'text-orange-600', bg: 'bg-white', border: 'border-orange-200', dot: 'bg-orange-600' },
    'Awaiting Decision': { color: 'text-indigo-600', bg: 'bg-white', border: 'border-indigo-200', dot: 'bg-indigo-600' },
    'Follow-Up Sent':    { color: 'text-cyan-600',   bg: 'bg-white', border: 'border-cyan-200',   dot: 'bg-cyan-600'   },
    'Offer Received':    { color: 'text-green-600',  bg: 'bg-white', border: 'border-green-200',  dot: 'bg-green-600'  },
    'Rejected':          { color: 'text-red-500',    bg: 'bg-white', border: 'border-red-200',    dot: 'bg-red-500'    },
    'Ghosted':           { color: 'text-gray-400',   bg: 'bg-white', border: 'border-gray-200',   dot: 'bg-gray-400'   },
  }

  const statBoxes = [
    { label: 'Total', key: 'All', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', dot: 'bg-blue-600' },
    ...enabledStatuses.map(status => ({
      label: status, key: status,
      ...(colorMap[status] || { color: 'text-gray-600', bg: 'bg-white', border: 'border-gray-200', dot: 'bg-gray-600' })
    }))
  ]
        ...
        {statBoxes.map(stat => {
          ...
          return (
            ...
            >
              ...
              <p className={`text-2xl font-bold mt-1 ${stat.color}`}>{counts[stat.key]}</p>
