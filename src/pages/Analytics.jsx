...
function Analytics() {
  ...
  const statusData = [
    ...
    {
      name: "Tech Test",
      count: applications.filter((a) => a.status === "Technical Test").length,
      fill: "#F59E0B",
    },
    { name: "Interview", count: interviews, fill: "#F97316" },
    {
      name: "Awaiting",
      count: applications.filter((a) => a.status === "Awaiting Decision")
        .length,
      fill: "#6366F1",
    },
  ...
  ].filter((d) => d.count > 0); // only show statuses with applications
  ...
  // Groups applications by month they were applied
  const timelineData = (() => {
    const monthMap = {};
  ...
  const funnelSteps = [
    ...
    },
    {
      ...
      rate:
        total > 0
          ? Math.round(
              (applications.filter((a) => a.status === "Phone Screen").length /
                total) *
                100,
            )
          : 0,
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-gray-400">
        ...
        <p className="text-sm mt-1">
          Add applications from the Dashboard to see your analytics
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-4">
            How many applications in each stage
          </p>
          {statusData.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Active vs rejected vs ghosted vs offers
          </p>
          {pieData.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-8">
              No data yet
              ...
              <div className="flex flex-col gap-2">
                {pieData.map((item) => (
                  <div key={item.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-gray-800 ml-auto pl-4">
                      {item.value}
                    </span>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-4">
            How many you applied each month
          </p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-5">
            How many applications convert at each stage
          </p>
          ...
          <div className="mt-5 bg-blue-50 rounded-xl p-4">
            ...
            <p className="text-xs text-blue-700 leading-relaxed">
              {offers > 0
                ? `🎉 Amazing! You converted ${offers} application${offers > 1 ? "s" : ""} to offers — that's a ${Math.round((offers / total) * 100)}% offer rate!`
                : interviews > 0
                  ? `You're getting interviews (${Math.round((interviews / total) * 100)}% rate) — focus on converting them to offers!`
                  : total >= 5
                    ? `You've applied to ${total} jobs. Keep pushing — interviews usually start coming after 10-15 applications.`
